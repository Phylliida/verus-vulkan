...
pub proof fn lemma_checked_nest_safe(
    ...
    requires
        ...
        can_nest_deeper(state, limits),
    ensures
        nested_cb_well_formed(nest_command_buffer_checked(state, limits), limits),
        nesting_depth(nest_command_buffer_checked(state, limits)) == nesting_depth(state) + 1,
        nest_command_buffer_checked(state, limits).state_isolated,
{
}
